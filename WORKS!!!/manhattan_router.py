...
Модель:
  ...
  - пины всех компонентов (и stub_end портов) регистрируются через
    add_pin(ref=...) и блокируются, кроме start/goal и своих own_refs.
...
class ManhattanRouter:
    ...
    def _cell_blocked(self, gx, gy, allow_cells, own_refs=None,
        ...
        Блокируются:
          ...
          * параллельные наложения проложенных проводов — только если
            они принадлежат ДРУГОЙ сети (своя сеть ходит по своим).

        Точки (пины и stub_end) блокируются раньше всего — их пересекать
        нельзя, поэтому они не могут стать "своей" клеткой.
        """
        # 1. Start/goal всегда разрешены.
        if allow_cells and (gx, gy) in allow_cells:
            return False
        # 2. Чужие пины — препятствие.
        if (gx, gy) in self._pins:
            pin_ref = self._pin_refs.get((gx, gy))
            if not (own_refs and pin_ref in own_refs):
                return True
        # 3. Проложенные провода: блокируем только параллельные наложения
        #    и только чужой сети. Своя сеть может ходить по своим проводам.
        cell = getattr(self, "_wire_cells", {}).get((gx, gy))
        if cell and cur_dir is not None:
            owner_net = cell.get(cur_dir)
            if owner_net is None or owner_net == _cur_net:
                pass  # клетка свободна в этом направлении или это своя сеть
            else:
                return True
        # 4. bbox: непрозрачны все.
    ...
    def _make_stub(self, g_pt, sdir, own_ref):
        ...
        Свой bbox — тоже препятствие (после выхода пин остаётся внутри
        своего bbox, но stub_end уже на свободе)."""
        gx, gy = g_pt
        ...
        if not _blocked(gx, gy):
            return (gx, gy)
        for step in range(1, 80):
            nx, ny = gx + dx * step, gy + dy * step
            if not _blocked(nx, ny):
                return (nx, ny)
        return (gx + dx * 80, gy + dy * 80)


    # ---------- A* ----------
    def route(self, p1, p2, own_refs=None,
        ...
        while heap:
            ...
            for dx, dy, dcur in DIRS:
                ...
                if not self._inside_bbox(nx, ny):
                    continue
                hv = "H" if dx != 0 else "V"
                if self._cell_blocked(nx, ny, allow, own_refs,
    ...
    def _log_failure_diag(self, p1, p2, g1, g2, best_cell, best_h,
                          own_refs, p1_owner, p2_owner, subst=False):
        ...
        for label, gpt in (("g1", g1), ("g2", g2)):
            ...
            for obs in self._obstacles:
                ...
                if own_refs and owner in own_refs:
                    continue
                if (x0 - c) <= wx <= (x1 + c) and (y0 - c) <= wy <= (y1 + c):
                    hit.append(owner or "?")
            if hit:
        ...
        for (pgx, pgy) in self._pins:
            ...
            if (xmin - self.grid <= wx <= xmax + self.grid and
                ...
                if own_refs and pref in own_refs:
                    continue
