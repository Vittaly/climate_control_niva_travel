...
# v58: локальные координаты пинов симметричных символов.
# Используется, если list_pins() у этого символа вернул пусто
# или неполный список (типично для Device:R, Device:C, Device:L).
SYMBOL_PIN_LAYOUT_V58 = {
    "Device:R":            [("1", 0.0,  3.81), ("2", 0.0, -3.81)],
    "Device:C":            [("1", 0.0,  3.81), ("2", 0.0, -3.81)],
    "Device:L":            [("1", 0.0,  3.81), ("2", 0.0, -3.81)],
    "Device:D":            [("1", 0.0,  3.81), ("2", 0.0, -3.81)],
    "Device:LED":          [("1", 0.0,  3.81), ("2", 0.0, -3.81)],
...
class SchematicGenerator:
    def __init__(self, main_yaml_path="./main.yaml"):
        ...
        # v50: stub_end каждого sheet-пина для роутера (MAIN_ROOT)
        self._sheet_pin_ends = {}
    ...
    def compile_sheet_to_kicad(self, page_name, page_info):
        ...
        # v50: сброс карты sheet-пинов для этой страницы
        self._sheet_pin_ends = {}
        ...
        for _ref16, _info16 in (components or {}).items():
            ...
            if not hasattr(self, "_sheet_sizes_v56"):
                self._sheet_sizes_v56 = {}
            self._sheet_sizes_v56[_sn16] = (_w16, _h16)
        ...
        if page_name == "MAIN_ROOT" and sheet_objs:
            ...
            for sname, (sobj, spos) in sheet_objs.items():
                ...
                def _add_pin(p, side, idx):
                    ...
                    # v57: сохраняем pin_pos и направление наружу.
                    # Роутер сам построит стаб через _make_stub.
                    self._sheet_pin_ends[(sname, net_label)] = {
                        "pin_pos": pin_pos,
                        "stub_end": stub_end,
                        "dir": (-1, 0) if side == "left" else (1, 0),
                        "_sheet_pin_v57": True,
                    }
                    ...
                    # v57: стаб НЕ рисуем явно — роутер построит
                    # его сам от pin_pos за bbox (как для обычных
                    # компонентов через _make_stub).
                    log.debug(
                        "    [SHEET-LABEL] %s.%s: add_label(%r, (%.3f,%.3f))",
                        sname, net_label, net_label,
                        pin_pos[0], pin_pos[1])
                    try:
                        sch.add_label(net_label, pin_pos)
                    except Exception as e:
        ...
        self._write_no_connects_via_api(sch, log=log)
    ...
    def _write_no_connects_via_api(self, sch, log=None):
        """v44c: no_connect через sch.no_connects.add.

        Сигнатура API:
            add(position: Union[Point, Tuple[float, float]],
                no_connect_uuid: Optional[str] = None)
              -> NoConnectElement

        Вызывается ДО sch.save — иначе в файл не попадёт.
        """
        if log is None:
            log = logging.getLogger("kicad_gen")
        nc_list = getattr(self, "_page_no_connects", []) or []
        self._page_no_connects = []
        if not nc_list:
            return

        col = getattr(sch, "no_connects", None)
        if col is None or not hasattr(col, "add"):
            log.warning("  [nc] sch.no_connects недоступен — пропуск")
            return

        getpos = getattr(sch, "get_component_pin_position", None)
        if getpos is None:
            log.warning("  [nc] get_component_pin_position недоступен — "
                        "пропуск")
            return

        placed = 0
        skipped = 0
        for ref, pin in nc_list:
            try:
                pos = getpos(ref, pin)
            except Exception as e:
                log.warning("  [nc] %s.%s: get_component_pin_position: %s",
                            ref, pin, e)
                skipped += 1
                continue
            if pos is None:
                log.warning("  [nc] %s.%s: позиция не найдена",
                            ref, pin)
                skipped += 1
                continue
            try:
                ...
                log.warning("  [nc] %s.%s: no_connects.add: %s",
                            ref, pin, e)
                skipped += 1

        log.info("  [nc] добавлено no_connect: %d, пропущено: %d",
                 placed, skipped)
    ...
    def _route_all(self, sch, comp_cache, final_placement, components, nets, page_name,
        ...
        pin_dir = {}
        all_pins = set()
        # v55: сначала собираем ВСЕ пины в _pins_per_comp, затем
        # вычисляем pin_dir по границам bbox пинов. Старая эвристика
        # «abs(dx) >= abs(dy)» ломалась на вытянутых символах: у
        # STM32F103R (корпус 30.48 x 91.44 мм) правые пины нижней
        # половины имеют dy > dx, получали направление (0,+1), и
        # стаб шёл ВНИЗ вдоль правой кромки, проходя сквозь все
        # нижележащие правые пины (PB8..PB15) — 8 разных сетей на
        # одной вертикали, короткое замыкание.
        _pins_per_comp = {}
        for ref, c in comp_cache.items():
            ...
            try:
                for p in c.list_pins():
                    ...
                    # НИКАКОГО snap(): API ставит пин в pos + local_pin
                    # без округления, и local_pin у ряда символов не кратен 1.27.
                    px = cx + lx
                    py = cy - ly
                    pin_xy[(ref, num)] = (px, py)
                    all_pins.add((round(px, 2), round(py, 2)))
                    pts.append((num, px, py))
            except Exception as e:
                log.warning("list_pins %s: %s", ref, e)
            # v58: если list_pins вернул пусто — достраиваем пины
            # из SYMBOL_PIN_LAYOUT_V58. Иначе компонент остаётся
            # без bbox, и A* ходит сквозь него и его пины.
            if not pts:
                info58 = components.get(ref, {}) or {}
                lib58 = self._guess_lib_id(ref, info58, component_types
                                          if 'component_types' in dir()
                                          else {})
                layout58 = SYMBOL_PIN_LAYOUT_V58.get(lib58)
                if layout58:
                    for _num58, _lx58, _ly58 in layout58:
                        _px58 = cx + _lx58
                        _py58 = cy - _ly58
                        pin_xy[(ref, _num58)] = (_px58, _py58)
                        all_pins.add((round(_px58, 2), round(_py58, 2)))
                        pts.append((_num58, _px58, _py58))
                    log.info("  [v58] %s (%s): list_pins пуст, "
                             "достроено %d пинов из layout",
                             ref, lib58, len(layout58))
            _pins_per_comp[ref] = pts

        # v55: pin_dir по границам bbox пинов.
        #   на левой   кромке → (-1, 0)
        #   на правой  кромке → (+1, 0)
        #   на нижней  кромке → (0, +1)
        #   на верхней кромке → (0, -1)
        #   углы / амбивалентные → старая эвристика по dx/dy.
        _TOL_V55 = 0.05
        for ref, pts in _pins_per_comp.items():
            if not pts:
                continue
            xs = [p[1] for p in pts]
            ys = [p[2] for p in pts]
            ...
            for num, px, py in pts:
                on_left = abs(px - x_min) < _TOL_V55
                on_right = abs(px - x_max) < _TOL_V55
                on_top = abs(py - y_min) < _TOL_V55
                on_bot = abs(py - y_max) < _TOL_V55
                if on_right and not on_left:
                    pin_dir[(ref, num)] = (1, 0)
                elif on_left and not on_right:
                    pin_dir[(ref, num)] = (-1, 0)
                elif on_bot and not on_top:
                    pin_dir[(ref, num)] = (0, 1)
                elif on_top and not on_bot:
                    pin_dir[(ref, num)] = (0, -1)
                else:
                    # Угловой или вырожденный случай — старая эвристика.
                    dx = px - (x_min + x_max) / 2.0
                    dy = py - (y_min + y_max) / 2.0
                    if abs(dx) >= abs(dy):
        ...
        # [diag_v58]
        for _dref in ('R1', 'R2', 'FB1', 'C8'):
            _dpts = [(n, x, y) for (r, n), (x, y) in pin_xy.items()
                     if r == _dref]
            if not _dpts:
                log.info("  [diag_v58] %s: НЕТ ПИНОВ в pin_xy", _dref)
                continue
            for _dn, _dx, _dy in sorted(_dpts):
                _ci = comp_cache.get(_dref)
                _cx = _cy = None
                try:
                    _cp = _ci.position
                    _cx, _cy = float(_cp.x), float(_cp.y)
                except Exception:
                    pass
                log.info(
                    "  [diag_v58] %s.%s: world=(%.3f,%.3f)  "
                    "comp_pos=(%s,%s)",
                    _dref, _dn, _dx, _dy,
                    f'{_cx:.3f}' if _cx is not None else '?',
                    f'{_cy:.3f}' if _cy is not None else '?')

        # v44c: пины без цепей -> no_connect.
        _used_pins = set()
        for _nm, _info in nets.items():
            for _n in _info.get("nodes", []):
                _c = _n.get("component")
                _p = str(_n.get("pin", ""))
                if _c:
                    _used_pins.add((_c, _p))
        self._page_no_connects = []
        for (_r, _n) in sorted(pin_xy.keys()):
            if _r not in comp_cache:
                continue
            if (_r, _n) in _used_pins:
                continue
            self._page_no_connects.append((_r, _n))
        if self._page_no_connects:
            log.info("  unconnected pins: %d (-> no_connect)",
                     len(self._page_no_connects))
            for _r, _n in self._page_no_connects[:60]:
                log.info("      NC %s #%s", _r, _n)
        ...
        for _r, _p in final_placement.items():
            ...
            _port_stub_pins += 1
        # v57: bbox листов (X_*) — препятствие, С MARGIN 1.0 мм
        # (как bbox обычных компонентов). Без margin пины листа
        # лежат на самой границе, и _make_stub может не сработать.
        _sheet_sizes = getattr(self, "_sheet_sizes_v56", {}) or {}
        # v57fix2: margin 0. pin_pos листов лежит на самой
        # границе bbox. С margin 1.0 pin_pos попадает внутрь
        # bbox+margin — раздувает поле и ломает _make_stub
        # (старт уже «внутри» — выходить некуда).
        # Для _cell_blocked pin_pos всё ещё попадает в зону
        # bbox + clearance (0.127), так что _make_stub сработает.
        _sheet_margin = 0.0
        for _ref57, _p57 in final_placement.items():
            if not str(_ref57).startswith("X_"):
                continue
            _sname57 = str(_ref57)[2:]
            _sz57 = _sheet_sizes.get(_sname57)
            if _sz57 is None:
                continue
            _sw57, _sh57 = _sz57
            router.add_obstacle(
                _p57["x"] - _sheet_margin,
                _p57["y"] - _sheet_margin,
                _p57["x"] + _sw57 + _sheet_margin,
                _p57["y"] + _sh57 + _sheet_margin,
                owner=_ref57)

        # v57: pin_pos -> пины роутера (как pin_xy компонентов).
        _sheet_stub_pins = 0
        for (_sname50, _nl50), _info57 in self._sheet_pin_ends.items():
            _pp57 = _info57["pin_pos"]
            router.add_pin(_pp57[0], _pp57[1], ref=f"X_{_sname50}")
            _sheet_stub_pins += 1
        # v57fix1: helper, который ищет направление пина и в
        # pin_dir (компоненты), и в self._sheet_pin_ends (листы).
        def _get_pin_dir_v57fix(_ref, _pin):
            _d = pin_dir.get((_ref, _pin))
            if _d is not None:
                return _d
            if str(_ref).startswith("X_"):
                _sn = str(_ref)[2:]
                _info = self._sheet_pin_ends.get((_sn, _pin))
                if _info is not None:
                    return _info["dir"]
            return None
        # v57fix1: sheet-пины НЕ добавляем в pin_dir. pin_xy для
        # них не заполнен (они не из comp_cache), а цикл by_dir
        # ниже обращается к pin_xy[ref, num]. Направление sheet-
        # пинов берём напрямую из self._sheet_pin_ends — см.
        # _get_pin_dir_v57fix ниже.
        log.info("  роутер: obstacles=%d, pins=%d (+%d port_stub, "
                 "+%d sheet_stub)",
                 len(router._obstacles), len(router._pins),
                 _port_stub_pins, _sheet_stub_pins)
        ...
        net_items = {}
        # v52: {net_name: set((gx,gy))} — все выводы цепи.
        # Сеть — свойство вывода: все выводы одной сети соединены,
        # маршрут внутри сети может проходить сквозь любой её вывод.
        net_own_pins = {}
        all_edges = []
        for net_name, info in nets.items():
            ...
            for n in info.get("nodes", []):
                ...
                elif str(c).startswith("X_"):
                    _sname50 = str(c)[2:]
                    _pin50 = str(n.get("pin"))
                    _info57c = self._sheet_pin_ends.get((_sname50, _pin50))
                    if _info57c is not None:
                        items.append((c, _pin50,
                                      _info57c["pin_pos"], "sheet"))
            ...
            # v52: соберём все клетки выводов этой цепи — они
            # «свои» для любого маршрута внутри этой цепи.
            _cells_v52 = set()
            for _it_v52 in items:
                _cells_v52.add((
                    int(round(_it_v52[2][0] / GRID)),
                    int(round(_it_v52[2][1] / GRID))))
            net_own_pins[net_name] = _cells_v52
            for i, j in edges:
                a = items[i]; b = items[j]
        ...
        for _negd, net_name, i, j in all_edges:
            ...
            _from_dir = (_get_pin_dir_v57fix(p_from[0], p_from[1])
                         if p_from[3] in ("comp", "sheet") else None)
            _to_dir = (_get_pin_dir_v57fix(p_to[0], p_to[1])
                       if p_to[3] in ("comp", "sheet") else None)
            _from_owner = p_from[0]
            _to_owner = p_to[0]
            # v52: own_pins — ВСЕ выводы текущей ЦЕПИ (net_name),
            # а не только два конца этого ребра. Сеть — свойство
            # вывода: все выводы одной сети соединены (MST строит
            # дерево на них же). Маршрут может пройти сквозь пин
            # своей сети — замыкания на чужую не будет. Чужие
            # сети остаются стеной.
            _own_pins = net_own_pins.get(net_name, set())
            ...
            if not _end_ok and _net_cells_here:
                ...
                if (_ex, _ey) in _net_cells_here:
                    _end_ok = True
            # own_terminal_v54: путь может закончиться только на
            # ПРОВОДЕ своей сети (T-стык). Пин без провода — не
            # терминал (иначе «коротыши» между соседними пинами).
            if not _end_ok:
                _ex = int(round(_end[0] / GRID))
                _ey = int(round(_end[1] / GRID))
                _wc54 = getattr(router, "_wire_cells", {}).get(
                    (_ex, _ey), {})
                if _wc54 and net_name in _wc54.values():
                    _end_ok = True
            if not _end_ok:
                # v37: ребро НЕ разведено — провод не рисуем вообще.
                # Ни в _net_cells, ни в connected его не добавляем,
                # чтобы следующие рёбра сети не считали p_to подключённым.
                # Ребро честно уходит в счётчик failed.
                _route_all_failed += 1
                log.error(
            ...
            # v49: в _net_cells кладём ТОЛЬКО стабы снаружи bbox.
            # Пин — внутри bbox своего компонента, его сюда класть
            # нельзя: следующее ребро подменит цель на него и
            # упрётся в bbox. Стаб по построению снаружи; но если
            # _make_stub не смог выйти (дошёл до cap) — тоже
            # отбрасываем, чтобы не засорять дерево.
            _stub_from_mm = getattr(router, "_last_stub_g1_mm", p_from[2])
            _stub_to_mm = getattr(router, "_last_stub_g2_mm", p_to[2])
            _clr = getattr(router, "clearance", 0.0)
            _obs = getattr(router, "_obstacles", [])
            def _in_any_bbox_v49(pt):
                _in_any_bbox_v57fix2 = True  # маркер патча
                _x, _y = pt[0], pt[1]
                for _o in _obs:
                    _x0, _y0, _x1, _y1 = _o[0], _o[1], _o[2], _o[3]
                    # v57fix2: bbox листов не мешает класть стаб
                    # в _net_cells. Стаб на границе листа — это
                    # корректная точка провода, следующее ребро
                    # сети должно уметь на нём финишировать.
                    _owner = _o[4] if len(_o) > 4 else None
                    if _owner and str(_owner).startswith("X_"):
                        continue
                    if (_x0 - _clr) <= _x <= (_x1 + _clr) and \
                       (_y0 - _clr) <= _y <= (_y1 + _clr):
                        return True
                return False
            for _lbl, _pt in (("from", _stub_from_mm),
                              ("to",   _stub_to_mm)):
                if _in_any_bbox_v49(_pt):
                    log.warning(
                        "  [net-cell-skip] %s: %s-стаб "
                        "(%.3f,%.3f) внутри bbox — не кладу в дерево",
                        net_name, _lbl, _pt[0], _pt[1])
                    continue
                cellset.add((int(round(_pt[0] / GRID)),
                             int(round(_pt[1] / GRID))))
            # own_terminal_v53: фактическая точка завершения пути —
            # тоже часть дерева. Если путь закончился на пине/проводе
            # своей сети (не в p_to), это T-узел; следующие рёбра
            # этой сети должны о него уметь финишировать.
            #
            # v57fix3: НО только если _end лежит ВНЕ всех bbox.
            # Точка внутри bbox — это pin_pos (пин компонента или
            # листа), а не провод. В дерево её класть нельзя: иначе
            # следующее ребро подменит на неё цель, _make_stub не
            # сработает, и pin_pos уйдёт в _last_stub_* (net-cell-skip).
            #
            # В отличие от _in_any_bbox_v49 (v57fix2), эта функция
            # НЕ пропускает bbox листов: для _end pin_pos листа —
            # это тоже пин, а не провод.
            def _inside_any_bbox_v57fix3(_pt):
                for _o in _obs:
                    _x0, _y0, _x1, _y1 = _o[0], _o[1], _o[2], _o[3]
                    if (_x0 - _clr) <= _pt[0] <= (_x1 + _clr) and \
                       (_y0 - _clr) <= _pt[1] <= (_y1 + _clr):
                        return True
                return False
            if not _inside_any_bbox_v57fix3(_end):
                cellset.add((int(round(_end[0] / GRID)),
                             int(round(_end[1] / GRID))))
...
# --- PATCHED-BY patch_v37.py ---

# --- PATCHED-BY patch_v39.py ---

# --- PATCHED-BY patch_v48 (generate). ---

# --- PATCHED-BY patch_v49.py ---

# --- PATCHED-BY patch_v44c.py ---

# --- PATCHED-BY patch_v50.py ---
# --- own_terminal_v54 ---
